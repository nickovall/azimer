...
export default function KpClient() {
...
function excluded(input: BuildingInput): string[] {
  ...
  // Гео — логистика по решению АЗИМЕР считается отдельно, в стоимость КП не входит
  if (!input.logisticsAdd) {
    list.push("Доставка / логистика материалов (рассчитывается отдельно)");
  }
  // Документы
